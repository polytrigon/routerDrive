...
/* Rename dialog (file list "Rename" -> #renameOverlay). Reuses the same
   .modal-overlay/.modal-box shell as the cut editor; only the per-file
   rows inside need their own rules. Narrower than the cut editor's box -
   it's a list of short text fields, not a drawing. */
.rename-box {
  max-width: 480px;
}
